...
NetworkSimulatorHelper::NetworkSimulatorHelper() {
  ...
  client_node_ = nodes.Get(0);
  server_node_ = nodes.Get(1);

  installNetDevice(client_node_, "eth0", getMacAddress("eth0"), Ipv4InterfaceAddress("193.167.0.2", "255.255.255.0"), Ipv6InterfaceAddress("fd00:cafe:cafe:0::2", 64));
  installNetDevice(server_node_, "eth1", getMacAddress("eth1"), Ipv4InterfaceAddress("193.167.100.2", "255.255.255.0"), Ipv6InterfaceAddress("fd00:cafe:cafe:100::2", 64));
}
...
void NetworkSimulatorHelper::Run(Time duration) {
  ...
  massageIpv6Routing(client_node_, server_node_);
  massageIpv6Routing(server_node_, client_node_);
...
Ptr<Node> NetworkSimulatorHelper::GetClientNode() const {
  return client_node_;
}

Ptr<Node> NetworkSimulatorHelper::GetServerNode() const {
  return server_node_;
}
